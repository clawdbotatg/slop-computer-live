 ...
 * real percentage.
 *
 * Uses node:http with a hand-rolled multipart body piped from
 * fs.createReadStream — NOT fetch + FormData + openAsBlob. Node 22's
 * fetch truncates a file-backed Blob's streamed length to 32 bits: a
 * >4 GiB recording sends only `size mod 4Gi` bytes, kubo waits forever
 * for the rest of the multipart body, and undici's body timeout kills
 * the request 5 minutes later ("finalize failed: Body Timeout Error",
 * pablosabbatella 2026-07-16 — first episode over 4 GiB).
 */
export async function pinToLocalIpfs(opts: {
  ...
  const url = new URL(`${opts.apiUrl}/api/v0/add?stream-channels=true&progress=true`);
  const request = url.protocol === "https:" ? httpsRequest : httpRequest;
  const boundary = `----slopIpfsAdd${randomBytes(16).toString("hex")}`;
  const head = Buffer.from(
    `--${boundary}\r\n` +
      `Content-Disposition: form-data; name="file"; filename="${basename(opts.file)}"\r\n` +
      `Content-Type: application/octet-stream\r\n\r\n`,
    "utf8",
  );
  const tail = Buffer.from(`\r\n--${boundary}--\r\n`, "utf8");
  const { size: fileSize } = await stat(opts.file);
  ...
  const res = await new Promise<IncomingMessage>((resolve, reject) => {
    ...
    req.on("error", reject);
    req.write(head);
    const src = createReadStream(opts.file);
    src.on("error", err => {
      reject(err);
      req.destroy(err);
    });
    src.pipe(req, { end: false });
    src.on("end", () => req.end(tail));
  });
  ...
  if (res.statusCode !== 200) {
    ...
    throw new Error(`kubo /api/v0/add ${res.statusCode}: ${text.slice(0, 200)}`);
  }
