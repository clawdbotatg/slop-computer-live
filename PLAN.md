...
1. EC2 region preference (us-east-1 vs us-west-2)?
2. SFU upgrade trigger — at what guest count does the mesh hurt enough to add MediaMTX as the SFU? Default: cross that bridge at 5+ simultaneous guests.
3. Persistent window layouts — store in relay memory only (lost on restart), or write to a tiny SQLite file on disk?
4. OBS browser-source resolution — 1920x1080 stage size? Higher for retina captures?
